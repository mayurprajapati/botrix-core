...
//import static org.monte.media.AudioFormatKeys.SilenceBugKey;
...
//	/**
//	 * outputTime and ffrDuration are needed for conversion of the video stream from
//	 * variable frame rate to fixed frame rate. FIXME - Do this with a CodecChain.
//	 */
...
//						COMPRESSOR_NAME_QUICKTIME_ANIMATION, DepthKey, 24, FrameRateKey, new Rational(15, 1)),
//				//
...
//	 *                     ENCODING_WHITE_CURSOR. The {@code SampleRateKey} can be
//	 *                     independent from the {@code screenFormat}. Specify null
//	 *                     if you don't want to capture the mouse cursor.
//	 * @param audioFormat  The audio format for audio capture. Specify null if you
//	 *                     don't want audio capture.
//	 */
//	public ScreenRecorder(GraphicsConfiguration cfg, Format fileFormat, Format screenFormat, Format mouseFormat,
...
//	 *                     don't want audio capture.
//	 */
//	public ScreenRecorder(GraphicsConfiguration cfg, Rectangle captureArea, Format fileFormat, Format screenFormat,
...
//				cursorImg = Images.toBufferedImage(
//						Images.createImage(org.monte.screenrecorder.ScreenRecorder.class, "images/Cursor.black.png"));
//				cursorImgPressed = Images.toBufferedImage(Images
//						.createImage(org.monte.screenrecorder.ScreenRecorder.class, "images/Cursor.black.pressed.png"));
//			} else if (this.mouseFormat.get(EncodingKey).equals(ENCODING_YELLOW_CURSOR)) {
//				cursorImg = Images.toBufferedImage(
//						Images.createImage(org.monte.screenrecorder.ScreenRecorder.class, "images/Cursor.yellow.png"));
//				cursorImgPressed = Images.toBufferedImage(Images.createImage(
//						org.monte.screenrecorder.ScreenRecorder.class, "images/Cursor.yellow.pressed.png"));
//			} else {
//				cursorImg = Images.toBufferedImage(
//						Images.createImage(org.monte.screenrecorder.ScreenRecorder.class, "images/Cursor.white.png"));
//				cursorImgPressed = Images.toBufferedImage(Images
//						.createImage(org.monte.screenrecorder.ScreenRecorder.class, "images/Cursor.white.pressed.png"));
//			}
...
//		Format videoInputFormat = screenFormat.prepend(MediaTypeKey, MediaType.VIDEO, EncodingKey,
//				ENCODING_BUFFERED_IMAGE, WidthKey, captureArea.width, HeightKey, captureArea.height, FrameRateKey,
//				videoRate);
//		Format videoOutputFormat = screenFormat
//				.prepend(FrameRateKey, videoRate, MimeTypeKey, fileFormat.get(MimeTypeKey))//
//				//
//				.append(//
//						WidthKey, captureArea.width, HeightKey, captureArea.height);
//
...
//		if (!videoInputFormat.intersectKeys(WidthKey, HeightKey)
//				.matches(videoOutputFormat.intersectKeys(WidthKey, HeightKey))) {
...
//	 * <p>
//	 * This implementation creates a file in the users "Video" folder on Windows, or
//	 * in the users "Movies" folders on Mac OS X.
//	 * <p>
//	 * You can override this method, if you would like to create a movie file at a
//	 * different location.
//	 *
//	 * @param fileFormat
//	 * @return the file
...
//		System.out.println("Capture delay in millisecs" + delay);
...
//
//		/**
//		 * Previously draw mouse location. This is used to have the last mouse location
//		 * at hand, when a new screen capture has been created, but the mouse has not
//		 * been moved.
//		 */
...
//				videoImg = new BufferedImage(this.captureArea.width, this.captureArea.height,
...
//				videoImg = new BufferedImage(this.captureArea.width, this.captureArea.height,
//						BufferedImage.TYPE_BYTE_INDEXED, Colors.createMacColors());
//			} else {
...
//
//		/**
//		 * Grabs a screen, generates video images with pending mouse captures and writes
//		 * them into the movie file.
//		 * 
//		 * @throws Exception
//		 */
...
//			try {
//				screenCapture = resize(getScreenImg());
...
//			if (mouseFormat != null && mouseFormat.get(FrameRateKey).intValue() > 0) {
//				while (!mouseCaptures.isEmpty()
//						&& mouseCaptures.peek().timeStamp.compareTo(new Rational(timeAfterCapture, 1000)) < 0) {
...
//							// draw cursor
//							if (prevMousePressed) {
//								videoGraphics.drawImage(cursorImgPressed, p.x + cursorOffset.x, p.y + cursorOffset.y,
//										null);
//							} else {
...
//		}
//
//		public static BufferedImage getScreenImg() throws Exception {
//			try {
//				if (G.webApp) {
//					Screenshot ss = new AShot().shootingStrategy(ShootingStrategies.viewportPasting(100))
//							.takeScreenshot(G.driver);
//					return ss.getImage();
//				} else if (G.windowsApp) {
...
//		}
//
//		private BufferedImage resize(BufferedImage screenImg) {
//			BufferedImage image = new BufferedImage(captureArea.width, captureArea.height, videoImg.getType());
//			Graphics g = image.createGraphics();
//			g.drawImage(screenImg, 0, 0, captureArea.width, captureArea.height, null);
//
//			if (StringUtils.isNotBlank(G.executionMetrics.getObjectNumber())) {
//				String s = ParseUtils.trimToEmpty(G.executionMetrics.getFlow().getProject()+"_"+
//						G.executionMetrics.getObjectNumber());
//				s = StringUtils.strip(s,"_");
//				g.setColor(Color.red);
//				g.setFont(new Font("Serif", Font.BOLD, 25));
//				FontMetrics fm = g.getFontMetrics();
//				int x = image.getWidth() - fm.stringWidth(s) - 5;
//				int y = image.getHeight() - fm.getHeight() - 5;
//				g.drawString(s, x, y);
//			}
//			g.dispose();
//			return image;
...
//
//	/**
//	 * Stops mouse capturing. Use method {@link #waitUntilMouseCaptureStopped} to
//	 * wait until the capturing stopped.
//	 */
...
//
//		/**
//		 * Previously captured mouse location. This is used to coalesce mouse captures
...
//			if (!info.getDevice().equals(captureDevice) || !captureArea.contains(p)) {
...
//		private Mixer mixer;
//
//		public AudioGrabber(Mixer mixer, Format audioFormat, int audioTrack, long startTime,
//				BlockingQueue<Buffer> queue) throws LineUnavailableException {
//			this.mixer = mixer;
//			this.audioFormat = audioFormat;
...
//					AudioFormatKeys.toAudioFormat(audioFormat));
//
//			if (mixer != null) {
...
//					buf.sampleCount = Math.max(0,
//							(int) Math.ceil(stopTS.subtract(buf.timeStamp).divide(buf.sampleDuration).floatValue()));
...
//
//		/**
//		 * Calculates the root-mean-square average of continuous samples. For four
//		 * samples, the formula looks like this:
//		 * 
//		 * <pre>
...
//	 * <p>
//	 * Stopping the screen recorder may take several seconds, because audio capture
//	 * uses a large capture buffer. Also, the MovieWriter has to finish up a movie
//	 * file which may take some time depending on the amount of meta-data that needs
//	 * to be written.
//	 */
...
//
//	/**
//	 * Writes a buffer into the movie. Since the file system may not be immediately
//	 * available at all times, we do this asynchronously.
//	 * <p>
//	 * The buffer is copied and passed to the writer queue, which is consumed by the
//	 * writer thread. See method startWriter().
//	 * <p>
//	 * AVI does not support a variable frame rate for the video track. Since we can
//	 * not capture frames at a fixed frame rate we have to resend the same captured
//	 * screen multiple times to the writer.
//	 * <p>
...
//	 *            {@code BufferedImage} in {@code buffer.data} and a {@code Point}
//	 *            in {@code buffer.header} with the recorded mouse location. The
//	 *            header is null if the mouse is outside the capture area, or mouse
...
//
//	/**
//	 * Gets the audio mixer used for sound input. Returns null, if the default mixer
//	 * is used.
//	 */
